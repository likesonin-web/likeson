 ...
 setupSocket(socket, user) {
  ...
  socket.on('driver_location', async (payload = {}) => {
    ...
    await handleDriverLocationUpdate({
      socketId: socket.id, userId, driverObjectId, role, name, payload,
  ...
  socket.on('care_location', async (payload = {}) => {
    ...
    await handleCaLocationUpdate({
      socketId: socket.id, userId, payload,
  ...
  socket.on('driver_status_update', async (payload = {}) => {
    ...
    const result = await handleRideStageTransition({
      userId, driverObjectId, name, payload,
  ...
  socket.on('verify_otp', async (payload = {}) => {
    ...
    const result = await handleOtpVerify({
      socketId: socket.id, userId, driverObjectId, name, payload,
    }).catch(e => ({ success: false, message: e.message }));
  ...
  socket.on('sos_trigger', async (payload = {}) => {
    const { allowed } = await canJoinRoom(
      { _id: userId, role, driverObjectId }, `booking:${payload.bookingId}`
    ).catch(() => ({ allowed: false }));
    ...
    try {
      ...
      const [booking, activeRide] = await Promise.all([
        ...
        Ride.findOne({ booking: bookingId, status: { $in: ACTIVE_RIDE_STATUSES } })
          .select('status rideStage activeNavigationTarget liveLocation driverSnapshot vehicleSnapshot trackingId pickup dropoff estimatedDistanceKm estimatedDurationMin waypoints')
          .lean(),
      ]);
      ...
      if (activeRide?.trackingId) {
        tracking = await RideTracking.findById(activeRide.trackingId)
          .select('currentEtaMinutes totalDistanceKm hasActiveSos milestones expectedRoutePolyline activeTarget careAssistantStatus careAssistantLiveLocation')
          .lean();
      }
