 ...
 * useDriverMarker.js
 *
 * Creates AdvancedMarkerElement ONCE. All updates mutate only:
 *   - marker.position        (lat/lng via rAF interpolation)
 *   - innerEl.style.transform (heading rotation, zero DOM rebuild)
 *
 * FIX vs original:
 *   - animatePosition uses closure refs — no stale state capture
 *   - destroyMarker cancels rAF before nulling refs
 *   - Speed dot color computed correctly
 */
...
export function useDriverMarker(mapRef, mapLoadedRef) {
  ...
  const ensureMarker = useCallback((lat, lng) => {
    ...
    pulse.style.cssText = `
      position:absolute;width:56px;height:56px;left:-28px;top:-28px;
      border-radius:50%;background:rgba(66,133,244,0.22);
      animation:drvPulse 2.2s ease-out infinite;pointer-events:none;
    `;

    const shadow = document.createElement('div');
    shadow.style.cssText = `
      position:absolute;width:40px;height:12px;left:-20px;top:14px;
      border-radius:50%;background:rgba(0,0,0,0.25);filter:blur(4px);pointer-events:none;
    `;

    const bubble = document.createElement('div');
    bubble.style.cssText = `
      position:absolute;width:46px;height:46px;left:-23px;top:-23px;
      background:linear-gradient(145deg,#5b9cf6,#3b82f6);border-radius:50%;
      border:2.5px solid rgba(255,255,255,0.9);
      display:flex;align-items:center;justify-content:center;
      box-shadow:0 4px 18px rgba(59,130,246,0.65),0 1px 4px rgba(0,0,0,0.3);
      will-change:transform;transform:rotate(0deg);pointer-events:none;z-index:2;
    `;
    bubble.innerHTML = `
    ...
    speedDot.style.cssText = `
      position:absolute;width:8px;height:8px;right:-2px;bottom:-2px;
      border-radius:50%;background:#22c55e;border:1.5px solid white;z-index:3;
    `;
  ...
  // rAF position interpolation — reads refs, never stale closures
  ...
  const updateMarker = useCallback((lat, lng, heading = 0, mapBearing = 0, speedKmh = 0) => {
    ...
    // Smooth heading, apply as CSS rotation only
    ...
    if (!currentPosRef.current) {
      ...
      const prev = targetPosRef.current || currentPosRef.current;
...
// ─────────────────────────────────────────────────────────────────────────────
// createStaticMarker  (pickup / dropoff pins)
// ─────────────────────────────────────────────────────────────────────────────
