...
export const requestOtpLogin = createAsyncThunk(
  ...
  async (identifier, { rejectWithValue }) => {
    try {
      const { data } = await API.post('/users/request-otp-login', { identifier });
...
export const forgotPassword = createAsyncThunk(
  ...
  async (identifier, { rejectWithValue }) => {
    try {
      const payload = typeof identifier === 'string' ? { identifier } : identifier;
...
export const registerDeviceToken = createAsyncThunk(
  ...
  async (payload, { rejectWithValue }) => {
    try {
      ...
      return rejectWithValue(extractError(err, 'Device token registration failed.'));
    }
  }
);
...
export const getWallet = createAsyncThunk(
  ...
  async ({ page = 1, limit = 20 } = {}, { rejectWithValue }) => {
    try {
      const { data } = await API.get(`/users/wallet?page=${page}&limit=${limit}`);
      return data.data;
    } catch (err) {
...
export const confirmPhoneVerification = createAsyncThunk(
  ...
  async (otp, { rejectWithValue }) => {
    try {
      const { data } = await API.post('/users/settings/verify-phone/confirm', { otp });
...
export const requestEmailChange = createAsyncThunk(
  ...
  async (newEmail, { rejectWithValue }) => {
    try {
      const { data } = await API.post('/users/settings/request-email-change', { newEmail });
...
export const deactivateAccount = createAsyncThunk(
  ...
  async (password, { rejectWithValue }) => {
    try {
      const { data } = await API.post('/users/settings/deactivate', { password });
      storage.clearAuth();
...
export const adminGetAllUsers = createAsyncThunk(
  ...
  async ({ page = 1, limit = 20, role, isBlocked, search } = {}, { rejectWithValue }) => {
    try {
...
export const adminGetUserCoins = createAsyncThunk(
  ...
  async (userId, { rejectWithValue }) => {
    try {
      ...
      return rejectWithValue(extractError(err, 'User coin details fetch failed.'));
    }
  }
);
      ...
      .addMatcher(
        ...
        (state, action) => {
          ...
          if (PROFILE_FULFILLED.has(action.type)) {
            ...
            if (source) {
              if (action.type === getProfile.fulfilled.type) {
                ...
                if (source.user)                  { state.user    = source.user; storage.patchUser(source.user); }
                if (source.profile !== undefined)   state.profile = source.profile;
              }
            }
          }
          ...
          if (action.type === getWallet.fulfilled.type && action.payload) {
            ...
            state.wallet.withdrawableBalance = action.payload.withdrawableBalance ?? 0;
            state.wallet.lockedBalance       = action.payload.lockedBalance       ?? 0;
            state.wallet.availableBalance    = action.payload.availableBalance    ?? 0;
            state.wallet.transactions        = action.payload.transactions        ?? [];
            state.wallet.pagination          = action.payload.pagination          ?? state.wallet.pagination;
          }
          ...
          if (action.type === redeemCoins.fulfilled.type && action.payload) {
            ...
            if (state.settings?.coins) {
              ...
              state.settings.coins.redeemed      = p.totalCoinsRedeemed ?? state.settings.coins.redeemed;
            }
          }
          ...
          if (action.type === unlinkGoogle.fulfilled.type) {
            if (state.user) {
              state.user = { ...state.user, googleAuth: { googleId: null, isVerified: false } };
              storage.patchUser(state.user);
            }
            if (state.settings?.verification) {
          ...
          if (action.type === acceptLegal.fulfilled.type && action.payload?.updated) {
            ...
            if (state.settings?.legal) {
              if (upd.termsAcceptedAt)         state.settings.legal.termsAcceptedAt         = upd.termsAcceptedAt;
              if (upd.privacyPolicyAcceptedAt) state.settings.legal.privacyPolicyAcceptedAt = upd.privacyPolicyAcceptedAt;
            }
            if (state.user) {
...
export const selectCoinsRupees   = (s) => +((s.user.user?.coins ?? 0) / 100).toFixed(2);
...
export const selectWithdrawableBalance = (s) => s.user.wallet.withdrawableBalance;
