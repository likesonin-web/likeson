...
import {
  ...
  // FIX: Renamed Image import to ImageIcon to avoid conflict with HTML <img> and Next.js <Image>
  Image as ImageIcon, Play, Users, MessageSquare, Archive, Bell, BellOff,
  Crown, UserMinus, Trash2, LogOut,
} from 'lucide-react';
...
export const MediaGalleryPanel = memo(({ conversationId, onClose }) => {
  ...
  useEffect(() => {
    if (conversationId) dispatch(fetchMediaMessages({ conversationId, type: mediaType }));
      ...
      <div className="flex-1 overflow-y-auto p-2">
        {mediaMessages.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-32 opacity-30">
            {/* FIX: Use ImageIcon alias to avoid JSX/component name conflict */}
...
export const ConversationDetails = memo(({ conv, currentUserId, onClose, onShowMedia }) => {
  ...
  const handleDelete = () => {
    if (!confirm('Delete this conversation for everyone? This cannot be undone.')) return;
      ...
      <div className="max-h-64 overflow-y-auto">
        {conversations.filter((c) => c._id !== activeConvId).map((c) => {
          ...
          return (
            <button
              key={c._id}
              className="flex items-center gap-3 w-full px-4 py-3 hover:bg-base-200 transition-colors text-left"
              onClick={async () => {
