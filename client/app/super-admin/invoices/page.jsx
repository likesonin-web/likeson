...
import {
  ...
  Calendar, TrendingUp, BarChart2, Wallet, CreditCard,
  Banknote, Hash, ArrowUpRight, Zap, X, Shield,
  ReceiptText, FileCheck, FileClock, FileX, FileSearch
} from 'lucide-react';
import {
  fetchPharmacyOrders,
...
const generateAndPrintInvoice = (order, { preview = false } = {}) => {
  const itemsHTML = (order.items ?? []).map((item, i) => `
    <tr>
      <td style="padding:10px 14px;font-size:12px;color:#374151;border-bottom:1px solid #f1f5f9;">${i+1}</td>
      <td style="padding:10px 14px;font-size:12px;color:#374151;border-bottom:1px solid #f1f5f9;">
        <strong style="color:#0f172a">${item.name}</strong>
      </td>
      <td style="padding:10px 14px;font-size:12px;color:#374151;border-bottom:1px solid #f1f5f9;text-align:right">${item.quantity}</td>
      <td style="padding:10px 14px;font-size:12px;color:#374151;border-bottom:1px solid #f1f5f9;text-align:right">₹${item.pricePerUnit?.toFixed(2)}</td>
      <td style="padding:10px 14px;font-size:12px;color:#374151;border-bottom:1px solid #f1f5f9;text-align:right">₹${(item.taxAmount??0).toFixed(2)}</td>
      <td style="padding:10px 14px;font-size:12px;font-weight:700;color:#0f172a;border-bottom:1px solid #f1f5f9;text-align:right">₹${item.totalPrice?.toFixed(2)}</td>
    </tr>`).join('');

  const payStatusColor = { Paid:'#16a34a', Pending:'#d97706', Failed:'#dc2626', Refunded:'#2563eb' };
  const payStatusBg    = { Paid:'#dcfce7', Pending:'#fef3c7', Failed:'#fee2e2', Refunded:'#dbeafe' };
...
<head>
  ...
  <style>
    ...
    *{margin:0;padding:0;box-sizing:border-box;}
    body{font-family:'Plus Jakarta Sans',sans-serif;background:#f8fafc;color:#0f172a;min-height:100vh;padding:32px;}
    .page{max-width:760px;margin:0 auto;background:#fff;border-radius:16px;overflow:hidden;box-shadow:0 20px 60px rgba(0,0,0,0.08);}
    .header{background:linear-gradient(135deg,#1d4ed8 0%,#0ea5e9 100%);padding:36px 40px;position:relative;overflow:hidden;}
    .header::before{content:'';position:absolute;top:-40px;right:-40px;width:200px;height:200px;background:rgba(255,255,255,0.08);border-radius:50%;}
    .header::after{content:'';position:absolute;bottom:-60px;right:80px;width:140px;height:140px;background:rgba(255,255,255,0.05);border-radius:50%;}
    .header-inner{display:flex;justify-content:space-between;align-items:flex-start;position:relative;z-index:1;}
    .brand{color:#fff;}
    .brand-name{font-size:28px;font-weight:900;letter-spacing:-1px;margin-bottom:4px;}
    .brand-sub{font-size:12px;opacity:0.7;font-weight:500;}
    .inv-meta{text-align:right;color:#fff;}
    .inv-meta h2{font-size:13px;font-weight:700;text-transform:uppercase;letter-spacing:2px;opacity:0.7;margin-bottom:8px;}
    .inv-meta .inv-num{font-size:22px;font-weight:900;letter-spacing:-0.5px;}
    .inv-meta .inv-date{font-size:12px;opacity:0.7;margin-top:4px;}
    .status-strip{background:#f8fafc;border-bottom:1px solid #e2e8f0;padding:16px 40px;display:flex;align-items:center;gap:12px;}
    .status-badge{padding:5px 14px;border-radius:999px;font-size:12px;font-weight:800;text-transform:uppercase;letter-spacing:0.5px;display:inline-flex;align-items:center;gap:6px;}
    .method-tag{font-size:11px;color:#64748b;background:#f1f5f9;border-radius:999px;padding:4px 12px;font-weight:700;border:1px solid #e2e8f0;}
    ...
    .party p{font-size:12px;color:#64748b;line-height:1.7;margin-bottom:2px;}
    .items-section{margin-bottom:28px;}
    .items-section h4{font-size:11px;font-weight:800;text-transform:uppercase;letter-spacing:1.5px;color:#94a3b8;margin-bottom:14px;}
    table.items{width:100%;border-collapse:collapse;border-radius:12px;overflow:hidden;border:1px solid #e2e8f0;}
    table.items thead tr{background:linear-gradient(135deg,#1d4ed8,#0ea5e9);}
    table.items thead th{padding:11px 14px;text-align:left;font-size:10px;font-weight:800;text-transform:uppercase;letter-spacing:0.8px;color:#fff;}
    table.items thead th:last-child,table.items thead th:nth-child(3),table.items thead th:nth-child(4),table.items thead th:nth-child(5){text-align:right;}
    table.items tbody tr:nth-child(even){background:#f8faff;}
    .billing-row{display:flex;justify-content:flex-end;margin-bottom:16px;}
    .billing-box{min-width:280px;background:#f8faff;border-radius:12px;padding:20px;border:1px solid #e0e7ff;}
    .b-row{display:flex;justify-content:space-between;font-size:12px;color:#64748b;margin-bottom:9px;}
    .b-total{display:flex;justify-content:space-between;font-size:17px;font-weight:900;color:#0f172a;border-top:2px solid #1d4ed8;padding-top:12px;margin-top:8px;}
    .b-total span:last-child{color:#1d4ed8;}
    .notes{background:#fffbeb;border:1px solid #fde68a;border-radius:10px;padding:14px 18px;font-size:12px;color:#92400e;margin-bottom:20px;display:flex;align-items:flex-start;gap:10px;}
    ...
    .footer .secure{display:flex;align-items:center;gap:6px;font-size:11px;color:#64748b;font-weight:600;}
    @media print {
...
<div class="page">
  ...
  <div class="status-strip">
    <span class="status-badge" style="background:${payStatusBg[pStatus]};color:${payStatusColor[pStatus]}">
      ${pStatus}
    </span>
    <span class="method-tag">${order.payment?.method ?? '—'}</span>
    <span class="method-tag">Delivery: ${order.delivery?.status ?? '—'}</span>
    ${order.billing?.promoCode ? `<span class="method-tag">Promo: ${order.billing.promoCode}</span>` : ''}
  </div>
  ...
  <div class="body">
    ...
    <div class="billing-row">
      <div class="billing-box">
        <div class="b-row"><span>Subtotal</span><span>₹${order.billing?.subTotal?.toFixed(2) ?? '0.00'}</span></div>
        <div class="b-row"><span>GST</span><span>₹${(order.billing?.gstAmount??0).toFixed(2)}</span></div>
        <div class="b-row"><span>Delivery Charges</span><span>₹${(order.billing?.deliveryCharges??0).toFixed(2)}</span></div>
        <div class="b-row"><span>Platform Fee</span><span>₹${(order.billing?.platformFee??0).toFixed(2)}</span></div>
        ${order.billing?.discountAmount ? `<div class="b-row"><span>Discount</span><span style="color:#16a34a;font-weight:700">-₹${order.billing.discountAmount.toFixed(2)}</span></div>` : ''}
        <div class="b-total"><span>Grand Total</span><span>₹${order.billing?.totalPayable?.toFixed(2) ?? '0.00'}</span></div>
      </div>
    </div>
  ...
  <div class="footer">
    ...
    <div class="secure">
      🛡️ Secured by LikesonHealth · support@likesonhealth.com
    </div>
  </div>
</div>
...
const ChartTooltip = memo(({ active, payload, label }) => {
  ...
  return (
    <div className="glass-card px-4 py-3 text-xs shadow-2xl border border-base-300">
      <p className="font-black text-base-content mb-1.5">{label}</p>
      {payload.map((p,i) => (
        <p key={i} style={{color:p.color}} className="mb-0.5">
          {p.name}: <strong>{String(p.name).includes('₹') ? `₹${Number(p.value).toLocaleString('en-IN')}` : p.value}</strong>
        </p>
      ))}
    </div>
  );
...
const InvoiceCard = memo(({ order, index, onView, onPrint }) => {
  const pStatus = order.payment?.status ?? 'Pending';
  const meta = PAYMENT_META[pStatus] ?? { color:'var(--info)' };
    ...
    >
      ...
      <div className="flex gap-2 pt-1 border-t border-base-300 opacity-0 group-hover:opacity-100 transition-all duration-200">
        <button
          ...
          className="flex-1 flex items-center justify-center gap-1.5 text-xs font-bold py-2 rounded-lg hover:bg-primary/10 text-primary transition-colors">
          <Eye size={13}/> Preview
        </button>
        <button
          onClick={e => { e.stopPropagation(); onPrint(order); }}
          className="flex-1 flex items-center justify-center gap-1.5 text-xs font-bold py-2 rounded-lg hover:bg-success/10 text-success transition-colors">
          <Printer size={13}/> Print
        </button>
        <button
          onClick={e => { e.stopPropagation(); generateAndPrintInvoice(order); }}
          className="flex-1 flex items-center justify-center gap-1.5 text-xs font-bold py-2 rounded-lg hover:bg-secondary/10 text-secondary transition-colors">
...
const InvoiceModal = memo(({ order, onClose }) => {
  ...
  return (
    <motion.div className="fixed inset-0 z-50 flex items-start justify-center p-4 overflow-y-auto"
      style={{paddingTop:'5vh'}}
      initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}}>
      <motion.div className="absolute inset-0 bg-black/60 backdrop-blur-md" onClick={onClose}/>
      <motion.div
        className="relative w-full max-w-2xl z-10 mb-8"
        style={{background:'var(--base-100)', borderRadius:'var(--r-box)', overflow:'hidden'}}
        initial={{scale:0.92, y:30}} animate={{scale:1, y:0}} exit={{scale:0.92, y:30}}
        transition={{type:'spring', damping:28}}>

        {/* Modal header */}
        <div className="sticky top-0 z-20 px-6 py-4 border-b border-base-300 flex items-center justify-between"
          style={{background:'color-mix(in srgb, var(--base-100) 95%, transparent)', backdropFilter:'blur(20px)'}}>
          <div className="flex items-center gap-3">
            <div className="p-2 rounded-xl bg-primary/10"><FileText size={18} className="text-primary"/></div>
            <div>
        ...
        <div className="p-6 space-y-5">
          ...
          <div className="rounded-xl overflow-hidden border border-base-300">
            <div className="p-5 text-white" style={{background:'linear-gradient(135deg, var(--primary), var(--secondary))'}}>
              <div className="flex justify-between items-start">
                ...
                <div className="text-right">
                  ...
                  <p className="text-xs opacity-70">{new Date(order.createdAt).toLocaleDateString('en-IN',{dateStyle:'long'})}</p>
                </div>
              </div>
            </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="glass-card p-4">
              ...
              <p className="text-xs text-base-content/50 mt-1 flex items-center gap-1"><Mail size={10}/>{order.customer?.email ?? '—'}</p>
              <p className="text-xs text-base-content/50 mt-1 flex items-center gap-1"><Phone size={10}/>{order.customer?.phone ?? '—'}</p>
              <p className="text-xs text-base-content/50 mt-1 flex items-center gap-1"><MapPin size={10}/>{order.delivery?.address?.line1}, {order.delivery?.address?.city}</p>
            </div>
            <div className="glass-card p-4">
                ...
                <thead>
                  <tr className="bg-primary/5">
                    {['#','Medicine','Qty','Unit Price','Tax','Total'].map(h => (
                      <th key={h} className="py-2.5 px-4 text-left font-bold uppercase tracking-wider text-base-content/40">{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {(order.items ?? []).map((item, i) => (
                    <tr key={i} className="border-t border-base-300 hover:bg-base-200/30">
                      <td className="py-2.5 px-4 text-base-content/50">{i+1}</td>
                      <td className="py-2.5 px-4 font-semibold text-base-content">{item.name}</td>
                      <td className="py-2.5 px-4 text-base-content/70">{item.quantity}</td>
                      <td className="py-2.5 px-4 text-base-content/70">₹{item.pricePerUnit?.toFixed(2)}</td>
                      <td className="py-2.5 px-4 text-base-content/50">₹{(item.taxAmount??0).toFixed(2)}</td>
                      <td className="py-2.5 px-4 font-black text-base-content">₹{item.totalPrice?.toFixed(2)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <div className="flex justify-end">
            <div className="glass-card p-5 w-full max-w-xs space-y-2">
              ...
              ].map(([label, val, isDiscount]) => val !== 0 && (
...
const InvoiceCardSkeleton = () => (
  <div className="glass-card p-5 space-y-4">
    <div className="flex justify-between"><div className="skeleton h-4 w-28 rounded"/><div className="skeleton h-5 w-16 rounded-full"/></div>
    <div className="flex items-center gap-2"><div className="skeleton w-8 h-8 rounded-full"/><div className="space-y-1.5 flex-1"><div className="skeleton h-3 w-32 rounded"/><div className="skeleton h-3 w-24 rounded"/></div></div>
    <div className="flex justify-between"><div className="skeleton h-6 w-24 rounded"/><div className="skeleton h-5 w-20 rounded-full"/></div>
  </div>
);
...
export default function InvoicePage() {
  ...
  const stats = useMemo(() => {
    ...
    orders.forEach(o => {
      if (o.payment?.status === 'Paid')    paid++;
      if (o.payment?.status === 'Pending') pending++;
      if (o.payment?.status === 'Failed')  failed++;
    });
    const totalRevenue = orders.filter(o => o.payment?.status === 'Paid').reduce((a,o) => a + (o.billing?.totalPayable??0), 0);
    return { paid, pending, failed, totalRevenue, total: orders.length };
  }, [orders]);
  ...
  const exportAll = useCallback(() => {
    const rows = [
      ...
      ...orders.map(o => `"${o.orderId}","${o.customer?.name??''}","${o.customer?.email??''}","${o.store?.name??''}",${o.items?.length??0},${o.billing?.subTotal??0},${o.billing?.gstAmount??0},${o.billing?.deliveryCharges??0},${o.billing?.totalPayable??0},"${o.payment?.method??''}","${o.payment?.status??''}","${new Date(o.createdAt).toLocaleDateString('en-IN')}"`)
    ].join('\n');
  ...
  return (
    <div className="min-h-screen p-4 md:p-6 space-y-6">
      ...
      <motion.div initial="hidden" animate="show" variants={stagger}
        ...
        <motion.div variants={fadeUp}>
          <h1 className="text-responsive-xl font-black text-base-content flex items-center gap-3">
            <div className="p-2.5 rounded-2xl" style={{background:'color-mix(in srgb, var(--accent) 15%, transparent)'}}>
        ...
        <motion.div variants={fadeUp} className="flex gap-2 flex-wrap">
            ...
            {[['grid','Grid'],['list','List']].map(([k,l]) => (
              <button key={k} onClick={() => setViewMode(k)}
                className={`px-4 py-1.5 text-xs font-bold rounded-lg transition-all ${
                  viewMode===k ? 'bg-accent text-accent-content shadow-md' : 'text-base-content/60 hover:text-base-content'
                }`}>{l}</button>
            ))}
          </div>
          ...
          <button onClick={exportAll} className="btn-primary-cta flex items-center gap-2 !py-2 !px-4">
      ...
      <motion.div initial="hidden" animate="show" variants={stagger}
        ...
        {[
          { title:'Total Invoices',   value: pagination.total.toLocaleString(), icon: FileText,  color:'var(--primary)' },
          { title:'Paid Invoices',    value: stats.paid,    icon: FileCheck, color:'var(--success)' },
          { title:'Pending',          value: stats.pending, icon: FileClock, color:'var(--warning)' },
          { title:'Revenue (Page)',   value:`₹${stats.totalRevenue.toLocaleString('en-IN',{maximumFractionDigits:0})}`, icon: IndianRupee, color:'var(--success)' },
        ...
        <div className="glass-card p-5">
          ...
          <ResponsiveContainer width="100%" height={160}>
            <AreaChart data={revenueByDay} margin={{top:4,right:4,bottom:0,left:4}}>
              ...
              <Area type="monotone" dataKey="revenue" stroke="var(--accent)" fill="url(#invRev)" strokeWidth={2.5} name="Revenue ₹"/>
            </AreaChart>
          </ResponsiveContainer>
        </div>
        ...
        <div className="glass-card p-4 space-y-3">
          <div className="flex flex-col sm:flex-row gap-3">
            <div className="relative flex-1">
              ...
              <input className="input-field w-full pl-9" placeholder="Search order ID, customer…"
            ...
            {showFilters && (
              <motion.div initial={{height:0,opacity:0}} animate={{height:'auto',opacity:1}} exit={{height:0,opacity:0}} className="overflow-hidden">
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3 pt-3 border-t border-base-300">
                  {[['Start Date','date','startDate'],['End Date','date','endDate'],['Min Amount ₹','number','minAmount'],['Max Amount ₹','number','maxAmount']].map(([l,t,k]) => (
                    <div key={k}>
                      <label className="text-xs font-semibold text-base-content/40 block mb-1">{l}</label>
                      <input className="input-field w-full" type={t} value={filters[k]} onChange={e => setFilter(k, e.target.value)}/>
                    </div>
                  ))}
                </div>
              </motion.div>
          ...
          <motion.div key="grid" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}}>
            {loading ? (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {Array(9).fill(0).map((_,i) => <InvoiceCardSkeleton key={i}/>)}
              </div>
            ) : orders.length === 0 ? (
              ...
              <motion.div initial="hidden" animate="show" variants={stagger}
                ...
                {orders.map((order, i) => (
                  <InvoiceCard key={order._id}
                    order={order} index={i}
                    onView={setActiveInvoice}
                    onPrint={generateAndPrintInvoice}/>
                ))}
              </motion.div>
            )}
          </motion.div>
                ...
                <tbody>
                  {loading ? Array(12).fill(0).map((_,i) => (
                    <tr key={i} className="border-b border-base-300">
                      {Array(9).fill(0).map((_,j) => <td key={j} className="py-4 px-4"><div className="skeleton h-4 rounded w-full"/></td>)}
                    </tr>
                  )) : orders.length === 0 ? (
                    <tr><td colSpan={9}>
                    ...
                    <motion.tr key={order._id}
                      ...
                      <td className="py-3.5 px-4 font-mono font-black text-primary text-xs">{order.orderId}</td>
                      ...
                      <td className="py-3.5 px-4 text-xs text-base-content/60">{order.store?.name ?? '—'}</td>
                      <td className="py-3.5 px-4 text-xs text-base-content/60">{order.items?.length ?? 0}</td>
                      <td className="py-3.5 px-4 font-black text-base-content">₹{order.billing?.totalPayable?.toLocaleString('en-IN') ?? '—'}</td>
                      <td className="py-3.5 px-4 text-xs text-base-content/60">{order.payment?.method ?? '—'}</td>
                      <td className="py-3.5 px-4"><PayBadge status={order.payment?.status}/></td>
                      <td className="py-3.5 px-4"><DelBadge status={order.delivery?.status}/></td>
                      <td className="py-3.5 px-4">
                        <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-all">
                          <button onClick={() => setActiveInvoice(order)}
                            className="p-1.5 rounded-lg hover:bg-primary/10 text-primary transition-colors" title="Preview">
                            <Eye size={14}/>
                          </button>
                          <button onClick={() => generateAndPrintInvoice(order)}
                            className="p-1.5 rounded-lg hover:bg-success/10 text-success transition-colors" title="Print">
                            <Printer size={14}/>
                          </button>
                          <button onClick={() => generateAndPrintInvoice(order)}
                            className="p-1.5 rounded-lg hover:bg-secondary/10 text-secondary transition-colors" title="Download">
                            <Download size={14}/>
                          </button>
                        </div>
                      </td>
            ...
            {pagination.pages > 1 && (
              <div className="flex items-center justify-between px-5 py-3 border-t border-base-300 bg-base-200/30">
                <p className="text-xs text-base-content/40">
                  Page {pagination.page} of {pagination.pages} · {pagination.total.toLocaleString()} invoices
                </p>
                <div className="flex gap-2">
      ...
      {viewMode === 'grid' && pagination.pages > 1 && (
        <div className="flex items-center justify-center gap-3">
          <button disabled={pagination.page<=1} onClick={() => setFilter('page', filters.page-1)}
            className="btn-secondary !py-2 !px-4 flex items-center gap-1.5 disabled:opacity-30">
          ...
          <button disabled={pagination.page>=pagination.pages} onClick={() => setFilter('page', filters.page+1)}
            className="btn-primary-cta !py-2 !px-4 flex items-center gap-1.5 disabled:opacity-30">
