...
import {
  GoogleMap, useJsApiLoader, Autocomplete, Marker,
...
import {
  fetchHospitals, fetchHospitalDoctors, checkHospitalAvailability,
  checkDoctorAvailability, fetchLabs, fetchLabById, fetchBookingOptions,
  fetchTransportEstimate, checkFollowUpEligibility, createFullCareRide, createDoctorConsultation,
...
function StepSchedule({
  ...
  // eslint-disable-next-line react-hooks/exhaustive-deps
    ...
    form.includeReturn, form.includeReturnHome, form.waitingMinutes,
  ]);
      ...
      {isPhysio && (
        <SCard title="Visit Type" icon={HeartPulse} accent="#10b981">
          ...
          {form.consultationType === 'homeVisit' && (
            <LocationPicker label="Your Home Address for Physio" required note="Physiotherapist will visit here"
              value={form.patientLocation} onChange={loc => set('patientLocation', loc)} error={errors.patientLocation} />
          )}
        </SCard>
      )}
        ...
        <div className="p-3 space-y-2 text-xs bg-base-100">
          {opNumber && (
            <div className="flex justify-between">
              ...
              <span className="font-black" style={PP}>{opNumber}</span>
            </div>
          )}
          {caAssigned?.name && (
            <div className="flex justify-between">
