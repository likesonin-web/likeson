...
/**
 * AppointmentsManagement.jsx — CLEAN v8
 *
 * REMOVED vs v7:
 * - All video call / VideoSDK / MeetingProvider logic
 * - All socket / socketService integration
 * - consultationSlice imports (fetchJoinDetails, startConsultation, endConsultation, logNetworkQuality, etc.)
 * - VideoCallModal, MeetingContent, MeetingControls, ParticipantTile, FloatingPip
 * - ReconnectOverlay, WaitingRoomPanel, EndConsultationModal
 * - ConsultationTimer, LiveEventFeed, ConsultationTimeline, NetworkQualityBars
 * - Consultation state machine / consultationReducer
 * - Session recovery for video
 *
 * CHANGED:
 * - Join video button → router.push link to /doctor/consultations/[bookingId]
 */
...
import {
  ...
  ClipboardList, PenLine, BadgeCheck, PhoneOff,
  Loader2, Bell,
  TrendingUp, Timer, BarChart2,
  Radio, UserCheck, UserX, StickyNote,
} from 'lucide-react';
...
const StatCard = memo(({ icon: Icon, label, value, sub, color, chartData, live }) => (
  ...
  >
    {live && (
      <span className="absolute top-3 right-3 flex items-center gap-1 text-xs text-success" aria-label="Live">
    ...
    <div className="flex items-start justify-between">
      ...
      <div className="p-2 rounded-xl" style={{ background: `color-mix(in srgb, ${color}, transparent 88%)` }} aria-hidden="true">
    ...
    <div className="relative" ref={ref}>
      ...
      <AnimatePresence>
        {open && (
          ...
          >
            <MenuItem
              icon={<Eye size={14} className="text-primary" aria-hidden="true" />}
              label="View Details"
              onClick={() => { setOpen(false); router.push(`/doctor/appointments/${booking._id}`); }}
            />
            {showVideo && (
              <MenuItem
                icon={<Video size={14} className="text-info" aria-hidden="true" />}
                label="Join Video Call"
                highlight="text-info"
                onClick={() => { setOpen(false); router.push(`/doctor/consultations/${booking._id}`); }}
              />
            )}
            <MenuItem
              icon={<PenLine size={14} className="text-accent" aria-hidden="true" />}
              label="Write Prescription"
              onClick={() => { setOpen(false); router.push(`/doctor/prescriptions/new?bookingId=${booking._id}`); }}
            />
            <MenuItem
              icon={<ClipboardList size={14} className="text-secondary" aria-hidden="true" />}
              label="View Prescriptions"
              onClick={() => { setOpen(false); router.push(`/doctor/prescriptions?bookingId=${booking._id}`); }}
            />
            {opRecord && (
              <MenuItem
                icon={<FileText size={14} className="text-info" aria-hidden="true" />}
            ...
            {canComplete && (
              <>
                <div className="h-px bg-base-300 mx-3 my-1" role="separator" />
                <MenuItem
                  icon={<CheckCircle2 size={14} className="text-success" aria-hidden="true" />}
                  label="Mark Complete"
        ...
        >
          ...
          >
            <div className="flex items-center gap-3 mb-5">
              <div className="p-2 rounded-xl bg-success/10" aria-hidden="true">
                <CheckCircle2 size={20} className="text-success" />
              </div>
              <div>
                <h3 className="font-bold text-base-content text-lg">Complete Consultation</h3>
                <p className="text-xs text-base-content/50">Fill clinical summary (all optional)</p>
              </div>
            </div>
            <div className="space-y-4">
              <div>
                <label htmlFor="complete-reason" className="label-text block mb-1.5">Reason for Visit</label>
                <input
                  id="complete-reason"
                  ref={firstInputRef}
              ...
              <div>
                <label htmlFor="complete-code" className="label-text block mb-1.5">
                  ICD-10 Code <span className="text-base-content/30 font-normal">(optional)</span>
                </label>
                <input
                  id="complete-code"
              ...
              <div>
                <label htmlFor="complete-notes" className="label-text block mb-1.5">Doctor Notes</label>
                <textarea
                  id="complete-notes"
...
export default function AppointmentsManagement() {
  ...
  const stats = useMemo(() => {
    ...
    return {
      ...
      online:   appointments.filter((a) => a.status === 'in_progress' && isOnlineBooking(a)).length,
    };
  }, [appointments, total]);
  ...
  return (
    <div data-theme="doctor" className="min-h-screen bg-base-100 text-base-content">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 py-6 space-y-6">
        ...
        {/* ── Stat Cards ───────────────────────────────────────────────────── */}
        <section aria-label="Appointment statistics" className="grid grid-cols-2 lg:grid-cols-4 xl:grid-cols-5 gap-4">
          ...
          <StatCard icon={Radio}        label="Live"      value={stats.online}    color="var(--success)" sub="In progress"     live={stats.online > 0} />
        </section>
        ...
        {/* ── Table ────────────────────────────────────────────────────────── */}
        <section aria-label="Appointments list" className="bg-base-100 border border-base-300 rounded-2xl overflow-hidden">
                  ...
                  <tr>
                    <td colSpan={8} className="py-16 text-center">
                      <div className="flex flex-col items-center gap-2 text-base-content/40">
                        <CalendarDays size={32} strokeWidth={1} aria-hidden="true" />
                  ...
                  {!loadingAppts && appointments.map((booking, i) => {
                    ...
                    return (
                      <motion.tr
                        ...
                        animate={{ opacity: 1, x: 0, transition: { delay: Math.min(i * 0.03, 0.3) } }}
                        exit={{ opacity: 0 }}
                        className="hover:bg-primary/5 cursor-default group"
                        aria-label={`Appointment for ${booking.patientInfo?.name ?? booking.customer?.name}`}
                      >
                        {/* Patient */}
                        <td>
                        ...
                        <td className="text-right">
                          <div className="flex items-center justify-end gap-1">
                            {/* Join video → simple link, no modal/SDK */}
                            {joinable && (
                              <button
                                className="btn btn-info btn-xs gap-1"
                                onClick={() => router.push(`/doctor/consultations/${booking._id}`)}
