...
function DaysBar({ daysLeft, status }) {
  ...
  const maxDays = 365;
  const pct     = status === 'missing' || status === 'expired'
    ? 0
...
function DocCard({ doc }) {
  ...
  const daysLabel =
    doc.status === 'missing'  ? 'Not submitted' :
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center justify-between gap-2 flex-wrap">
            ...
            <span
              ...
              style={{
                background: `color-mix(in srgb, ${cfg.color}, transparent 82%)`,
                color:      cfg.color,
        ...
        {!loading && compliance && (
          <>
            ...
            >
              {/* decorative blobs */}
              <div
                className="absolute -top-8 -right-8 w-44 h-44 rounded-full opacity-15"
                style={{ background: 'radial-gradient(circle, white 0%, transparent 70%)' }}
              />
              <div
                className="absolute -bottom-10 -left-12 w-52 h-52 rounded-full opacity-10"
                style={{ background: 'radial-gradient(circle, white 0%, transparent 70%)' }}
              />
