 ...
 *
 * React context for the /consultation Socket.IO namespace.
 * Mirrors backend consultationSocketService.js event contract.
 *
 * USAGE:
 *   // Wrap consultation pages only — NOT app-root (keep isolated from ride socket)
 *   <ConsultationSocketProvider token={jwtToken} consultationId={id}>
 *     <ConsultationRoom />
 *   </ConsultationSocketProvider>
 *
 *   // Inside any child:
 *   const { sendChat, toggleMic, admitPatient, ...state } = useConsultation();
 *
 * Hooks exported:
 *   useConsultation()         — full context
 *   useConsultationChat()     — chat messages + sendChat
 *   useWaitingRoom()          — waiting room queue management (doctor)
 *   useParticipants()         — live participant list
 *   useConsultationMedia()    — mic/camera/screen/recording controls
 *   useConsultationHealth()   — connection status + latency ping
 */
