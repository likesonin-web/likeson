...
import React, { useState, useMemo, useEffect, useCallback, memo, useRef, useId } from 'react';
...
// --- Metadata ---
// Note: In Next.js App Router, metadata should ideally be exported from a layout.js or page.js (Server Component),
// not a "use client" component. If this is page.js, consider moving metadata to a parent server layout.
  ...
  <div className="relative w-56 h-56 mx-auto lg:mx-0 z-10 pointer-events-none select-none" aria-hidden="true">
    ...
    >
      <svg viewBox="0 0 200 200" fill="none" xmlns="http://www.w3.org/2000/svg" className="w-full h-full drop-shadow-xl">
        <motion.g animate={{ y: [0, -5, 0], opacity: [0.7, 1, 0.7] }} transition={{ duration: 3, repeat: Infinity, ease: "easeInOut" }}>
        ...
        <motion.path d="M60 80 L45 45 L80 65" fill="var(--base-200)" stroke="var(--base-300)" strokeWidth="3" strokeLinejoin="round" animate={{ rotate: [-2, 2, -2] }} transition={{ duration: 4, repeat: Infinity, ease: "easeInOut" }} style={{ transformOrigin: '60px 80px' }} />
        <motion.path d="M140 80 L155 45 L120 65" fill="var(--base-200)" stroke="var(--base-300)" strokeWidth="3" strokeLinejoin="round" animate={{ rotate: [2, -2, 2] }} transition={{ duration: 4, repeat: Infinity, ease: "easeInOut" }} style={{ transformOrigin: '140px 80px' }} />
        <motion.path d="M55 110 C35 130, 40 160, 50 160 C60 160, 60 130, 60 110" fill="var(--primary)" opacity="0.9" animate={{ rotate: [0, 8, 0] }} transition={{ duration: 3, repeat: Infinity, ease: "easeInOut" }} style={{ transformOrigin: '55px 110px' }} />
        <motion.path d="M145 110 C165 130, 160 160, 150 160 C140 160, 140 130, 140 110" fill="var(--primary)" opacity="0.9" animate={{ rotate: [0, -8, 0] }} transition={{ duration: 3, repeat: Infinity, ease: "easeInOut" }} style={{ transformOrigin: '145px 110px' }} />
        ...
        <motion.g animate={{ rotate: [-1, 2, -1] }} transition={{ duration: 6, repeat: Infinity, ease: "easeInOut" }} style={{ transformOrigin: '100px 90px' }}>
          ...
          <motion.g animate={{ scaleY: [1, 1, 0.1, 1, 1] }} transition={{ duration: 5, repeat: Infinity, times: [0, 0.45, 0.5, 0.55, 1] }} style={{ transformOrigin: 'center' }}>
...
DocOwl.displayName = 'DocOwl';

// ─── Animated Creature 2: PillBot ───────────────
const PillBot = memo(() => (
  <motion.div
  ...
  >
    <svg viewBox="0 0 100 100" className="w-full h-full drop-shadow-md">
      ...
      <motion.g animate={{ scaleY: [1, 0.1, 1] }} transition={{ duration: 4, repeat: Infinity, times: [0, 0.05, 0.1], repeatDelay: 3 }} style={{ transformOrigin: 'center' }}>
...
PillBot.displayName = 'PillBot';

// ─── Animated Creature 3: Hearty ───────────────
const Hearty = memo(() => (
  <motion.div
    animate={{ y: [4, -4, 4] }}
  ...
  >
    <svg viewBox="0 0 100 100" className="w-full h-full drop-shadow-md">
      <motion.g animate={{ scale: [1, 1.15, 1, 1.15, 1] }} transition={{ duration: 1.5, repeat: Infinity, times: [0, 0.15, 0.3, 0.45, 1] }} style={{ transformOrigin: '50px 50px' }}>
      ...
      <motion.circle cx="50" cy="50" r="42" stroke="var(--error)" strokeWidth="1.5" strokeDasharray="6 6" fill="none" opacity="0.3" animate={{ rotate: 360 }} transition={{ duration: 12, repeat: Infinity, ease: "linear" }} style={{ transformOrigin: "50px 50px"}} />
      <motion.path d="M80 20 L82 15 L87 13 L82 11 L80 6 L78 11 L73 13 L78 15 Z" fill="var(--accent)" animate={{ scale: [0, 1, 0], rotate: 180 }} transition={{ duration: 3, repeat: Infinity }} />
    </svg>
  </motion.div>
));
Hearty.displayName = 'Hearty';
...
const STATIC_FAQS = [
  { _id: "faq-1", category: "Medical Transportation", question: "How do I track my ambulance in real-time?", answer: "Once your Full Care Ride or Ambulance is dispatched, you will receive an SMS and an app notification with a live GPS tracking link. You can monitor the vehicle's exact location and estimated time of arrival (ETA) directly from your dashboard." },
  { _id: "faq-2", category: "Doctor Consultation", question: "Are the doctors on Likeson board-certified?", answer: "Absolutely. Every doctor on our platform undergoes a rigorous 5-step background check. We verify their medical council registration, educational qualifications, and clinical experience to ensure you receive premium, reliable care." },
  { _id: "faq-3", category: "Pharmacy Services", question: "How fast is the pharmacy delivery?", answer: "For standard prescriptions, we guarantee express delivery within 2 hours. Life-saving and cold-chain medicines are transported in specialized temperature-controlled vehicles to maintain complete efficacy." },
  { _id: "faq-4", category: "Diagnostics", question: "Can I get my lab samples collected at home?", answer: "Yes! Our NABL-accredited lab partners offer free home sample collection. A certified phlebotomist will visit your home at your scheduled time, and digital reports will be uploaded directly to your Likeson account within 6 to 48 hours." }
];
...
const AccordionItem = memo(({ id, question, answer, isOpen, onClick }) => {
  const uid = useId();
  const headerId = id ? `faq-question-${id}` : `faq-question-${uid}`;
  const contentId = id ? `faq-answer-${id}` : `faq-answer-${uid}`;
...
export default function FAQPage() {
  ...
  // FIX: Track if we have initiated a fetch to avoid loops if API returns []
  const hasAttemptedFetch = useRef(false);
  // FIX: Prevent hydration mismatch with a mounted state
  ...
  useEffect(() => {
    // Only attempt fetch once per mount lifecycle if Redux is empty
    if (!hasAttemptedFetch.current && (!reduxFaqs || reduxFaqs.length === 0)) {
      hasAttemptedFetch.current = true;
      const handleFetch = () => dispatch(fetchFAQs({ isActive: true }));
  ...
  const faqs = useMemo(() => {
    return reduxFaqs?.length > 0 ? reduxFaqs : STATIC_FAQS;
  }, [reduxFaqs]);
  ...
  // Avoid rendering dynamic content that differs from server output during SSR
              ...
              <div aria-live="polite" className="min-h-[400px]">
                {isLoading ? (
                  <div className="space-y-4" aria-hidden="true">
                    {[1, 2, 3, 4, 5].map(i => <div key={i} className="skeleton h-20 w-full rounded-[var(--r-box)] bg-base-200 animate-pulse" />)}
                  </div>
